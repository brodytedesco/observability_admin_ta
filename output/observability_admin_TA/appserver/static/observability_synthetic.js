...
function detector_functions ($, _, mvc) {
    ...
    function get_realm() {
        ...
        if (passwordEntry) {
            ...
            var tokens = mvc.Components.get("default");
            var idValue = tokens.get("id");
            var typeValue = tokens.get("type");
            if (idValue!="$result.id$"){
            set_realm_dashboard(passwordValue,idValue,typeValue)
            }
    ...
    function set_realm_dashboard(realm,id,type){
        var myLink = document.getElementById("link_to_object");
        myLink.href = "https://app."+realm+".signalfx.com/#/synthetics/tests/view/"+type+"/"+id;
